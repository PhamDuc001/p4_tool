...
print("=== DEEP TEST: Property Update with Conditional Blocks ===")
...
try:
    ...
    if success:
        ...
        with open(temp_file, 'r') as f:
            ...
            # Verify no merged conditional blocks
            lines = result.split('\n')
            conditional_blocks = []
            current_block = None
            for i, line in enumerate(lines):
                if "ifneq" in line and "PRODUCT_PROPERTY_OVERRIDES" in line:
                    current_block = {'start': i, 'type': 'if', 'content': []}
                elif "else" == line.strip() and current_block:
                    current_block['else'] = i
                elif "endif" == line.strip() and current_block:
                    current_block['end'] = i
                    conditional_blocks.append(current_block)
                    current_block = None
                elif current_block and "PRODUCT_PROPERTY_OVERRIDES" in line:
                    current_block['content'].append(line)
            
            if len(conditional_blocks) >= 2:  # Should have at least 2 conditional blocks
                print("[SUCCESS] Conditional blocks structure maintained!")
            else:
                print("[WARNING] May have issues with conditional block structure")
