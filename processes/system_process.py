...
from core.p4_operations import (
    ...
    is_workspace_like, find_device_common_mk_from_workspace,
...
def resolve_input_to_device_common_path(user_input, log_callback=None):
    ...
    # If it's a workspace
        ...
        try:
            ...
            resolved_path = find_device_common_mk_from_workspace(user_input, log_callback)
            if resolved_path:
                if log_callback:
...
def find_filtered_samsung_vendor_paths(all_view_paths, log_callback=None):
    """
    ENHANCED: Find vendor/samsung paths with improved filtering logic
    Priority 1: paths ending with /vendor/samsung/...
    Priority 2: paths containing /vendor/samsung/system/... or /vendor/samsung/system/rscmgr/...
    Returns list of matching depot paths with priority-based filtering
    """
    if log_callback:
        log_callback("[SYSTEM] Searching for enhanced filtered vendor/samsung paths...")
    
    samsung_paths = []
    valid_prefixes = ["//PROD_VINCE", "//MODEL", "//BENI", "//PROD_BENI", "//VINCE"]
    
    # Step 1: Find all paths containing vendor/samsung
    candidate_paths = []
    for path in all_view_paths:
        path_starts_with_valid_prefix = any(path.startswith(prefix) for prefix in valid_prefixes)
        
        if path_starts_with_valid_prefix and '/vendor/samsung/' in path:
            candidate_paths.append(path)
    
    # Step 2: Apply enhanced filtering logic with priority
    for path in candidate_paths:
        samsung_base_path = None
        
        # Priority 1: paths ending with /vendor/samsung/...
        if path.endswith('/vendor/samsung/...'):
            samsung_base_path = path.rstrip('...')  # Remove ... to get /vendor/samsung/
            
        # Priority 2: paths with /vendor/samsung/system/... or /vendor/samsung/system/rscmgr/...
        elif '/vendor/samsung/system/' in path:
            # Extract base vendor/samsung path
            match = re.search(r'(.+/vendor/samsung/)', path)
            if match:
                samsung_base_path = match.group(1)
        
        if samsung_base_path and samsung_base_path not in samsung_paths:
            samsung_paths.append(samsung_base_path)
    
    if log_callback:
        if samsung_paths:
            log_callback(f"[OK] Found {len(samsung_paths)} enhanced vendor/samsung paths")
            for path in samsung_paths:
                log_callback(f"[RESULT] {path}")
        else:
            log_callback("[WARNING] No enhanced vendor/samsung paths found")
...
def process_target_workspace_enhanced(workspace_or_path, category, vince_rscmgr_filename, vince_rscmgr_path, 
    ...
    Now supports both workspace and depot path input with enhanced Samsung vendor path filtering
    ...
    try:
        # Resolve input to device_common.mk path using enhanced logic
        ...
        # Get all view paths - handle both workspace and depot path cases with enhanced logic
        all_view_paths = []
        
        # If input was workspace, get view paths from workspace using enhanced approach
        ...
        if device_common_needs_update:
            ...
            checkout_file_silent(device_common_path, current_changelist_id, log_callback=None)
        ...
        # Find filtered samsung vendor paths with enhanced priority logic
        samsung_paths = find_filtered_samsung_vendor_paths(all_view_paths, log_callback)
        
        # Enhanced fallback: If no samsung paths found from workspace, construct from device_common path
        if not samsung_paths and device_common_path:
            if log_callback:
                log_callback("[ENHANCED-FALLBACK] Constructing samsung paths from device_common.mk path...")
            
            # Extract base path from device_common.mk path with enhanced matching
            base_path_patterns = [
                r'^(.+/vendor/samsung/)',  # Standard vendor/samsung path
                r'^(.+)/device/[^/]+_common/',  # Extract base path before device/
            ]
            
            for pattern in base_path_patterns:
                match = re.search(pattern, device_common_path)
                if match:
                    if '/vendor/samsung/' in match.group(1):
                        base_samsung_path = re.search(r'(.+/vendor/samsung/)', match.group(1)).group(1)
                        samsung_paths = [base_samsung_path]
                        if log_callback:
                            log_callback(f"[ENHANCED-FALLBACK] Constructed samsung path: {base_samsung_path}")
                        break
        ...
        if target_rscmgr_path:
            ...
            if target_lines == vince_lines:
                ...
                checkout_file_silent(target_rscmgr_path, current_changelist_id, log_callback=None)
            ...
            if samsung_paths:
                ...
                if log_callback:
                    log_callback(f"[WARNING] No enhanced filtered samsung paths found in {category}, cannot create rscmgr file")
...
def run_system_process(beni_input, vince_input, flumen_input, rel_input,
    ...
    """
    FIXED: Execute system bringup process that processes ALL auto-resolved targets
    """
    try:
        # ============================================================================
        # STEP 1: VALIDATE AND AUTO-RESOLVE INPUTS
        # ============================================================================
        log_callback("[SYSTEM] Starting FIXED system bringup process with auto-resolve support...")
        ...
        # ============================================================================
        log_callback("\n[SYSTEM] Processing VINCE input (master source) with enhanced filtering...")
        ...
        # Find VINCE rscmgr file - enhanced logic for both workspace and depot path input
        vince_rscmgr_path = None
        
        # If VINCE input was workspace, use workspace logic with enhanced filtering
        if is_workspace_like(final_vince_input):
            try:
                _, vince_all_paths = find_device_common_mk_path(final_vince_input, log_callback)
        ...
        # Enhanced fallback: If not found or depot path input, try path-based construction
        if not vince_rscmgr_path:
            if log_callback:
                log_callback("[ENHANCED-FALLBACK] Attempting enhanced path-based rscmgr file construction...")
        ...
        log_callback(f"[OK] VINCE processing completed with enhanced filtering. Master rscmgr file: {vince_rscmgr_filename}")

        if progress_callback:
            progress_callback(60)

        # ============================================================================
        # STEP 4: PROCESS ALL RESOLVED TARGETS (FIXED TO USE AUTO-RESOLVED VALUES)
        # ============================================================================
        ...
        # Process ALL resolved targets with enhanced filtering
        ...
        for category, target_input in all_resolved_targets:
            log_callback(f"\n[SYSTEM] Processing {category} target with enhanced mixed input + filtering logic...")
        ...
        # ============================================================================
        # STEP 5: SUMMARY - UPDATED TO SHOW ALL PROCESSED TARGETS
        # ============================================================================
        processed_targets = [cat for cat, _ in all_resolved_targets]
        log_callback(f"\n[SYSTEM] FIXED system bringup process completed successfully!")
        log_callback(f"[SUMMARY] Processed ALL resolved targets with auto-resolve + enhanced filtering: {', '.join(processed_targets)}")
        log_callback(f"[SUMMARY] Master rscmgr file: {vince_rscmgr_filename}")
        log_callback(f"[SUMMARY] Enhanced Samsung vendor path filtering applied")
        ...
        log_callback(f"[ERROR] FIXED system process with auto-resolve failed: {str(e)}")
        if error_callback:
